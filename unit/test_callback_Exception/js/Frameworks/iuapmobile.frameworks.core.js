	...
	try{		
		...
		if(typeof jsonArgs == "string"){
			...
			if(jsonArgs["callback"] && $isFunction(jsonArgs["callback"])){
				...
				window[strFn.substring(0,strFn.indexOf("("))] = function (sender, args){
					alert(typeof sender);
					alert(typeof args);
					$alert(sender);
					$alert(args);
					var func = $__cbm[strFn];
					func(sender, args);	
					delete $__cbm[strFn];
					alert("del");
					alert(typeof $__cbm[strFn]);
				
				}
				jsonArgs["callback"] = strFn;				
			}else if(jsonArgs["callback"] && typeof(jsonArgs["callback"]) == "string"){
				// callback:"mycallback()"
				var end = jsonArgs["callback"].indexOf("(");
				var cbName = jsonArgs["callback"].substring(0,end);
				var callbackFn = eval(cbName);
				if(callbackFn != "function"){
					alert(cbName + " is not a callback function");
					return;
				}
				var strFn = "fun" + uuid(8, 16) + "()";//anonymous method
				while($__cbm[strFn]){
				...
				$__cbm[strFn] = callbackFn;
				window[strFn] = callbackFn;
				jsonArgs["callback"] = strFn;	
			}
